---
title: ABUNDANCE, HABITAT ASSOCIATIONS, AND MIGRATORY CONNECTIVITY OF BIRD SPECIES
  OF SPECIAL CONCERN ON JOINT BASE ELMENDORF-RICHARDSON, ALASKA
author:
...
![Core boreal wetland survey units selected in 2020. ](./figures/SurveyUnits.jpg){width=80%}
...
![Point count locations within the proposed expansion area of Eagle River Flats](./figures/ERFExpansion.jpg){width=80%}
...
![ARU locations on Eagle River Flats in 2019 and 2020. Two units were placed in the same location on the eastern edge of Eagle Rier Flats in both years](./figures/ARU_locations.jpg){width=80%}
...
![Left: Recently hatched Lesser Yellowlegs chicks in nest bowl at Ammo Country wetlands. Right: Placement of a TinyTag data logger probe in the center of a Lesser Yellowlegs nest ](./figures/leye_probe.jpg){width=80%}
...
![Left: Leg bands: Metal, plastic and alphanumeric flags. Right: Measuring the diagonal tarsus of a Lesser Yellowlegs](./figures/bands.jpg){width=80%}
\newpage
...
![Blackpoll Warbler are a Neotropical migrant that breeding in the boreal forests of Alaska and Canada. This is a late arrive species that was only detected during the second survey period. The male's striking black and white plumage is contrary to the females' monocramatic olive-green plumage](./figures/blpw.jpg){width=80%}
...
\caption[Summary of Species of Special Concern detected on unit during two survey periods on Joint Base Elmendorf-Richardson, 2020.]{Summary of Species of Special Concern detected on unit during two survey periods on Joint Base Elmendorf-Richardson, 2020.}


```{r table1, out.width="60%", eval=TRUE, echo=FALSE, message=FALSE}
...
kable(format="html",
  ...
  as_image(table1,file="table1.png", width = 4)
...
![Left: Blackpoll Warbler and Rusty Blackbird nest within mixed forests along the edges of wetlands. Right: Sandpiper species forage in the emergent vegetation along the edges of lakes and ponds.](./figures/survey_habitat.jpg){width=80%}
...
\caption[Avian species detected on the proposed expansion area of Eagle River Flats during two point counts periods, 2020.]{Avian species detected on the proposed expansion area of Eagle River Flats during two point counts periods, 2020.}


```{r table2, out.width="80%", eval=TRUE, echo=FALSE, message=FALSE}
...
kable(format="html",
  ...
  as_image(table2,file="table2.png", width = 4)
...
![A failed Lesser Yellowlegs nest found on the Ammo Country wetlands at Joint Base Elmendorf-Richardson, 2020. The 4mm diameter fissures and egg frangments suggest the nest was depredated by an avian predator.](./figures/amco_egg.jpg){width=50%}
...
plot1 = plot(accurve1, lty= 1, ci.type="line", col="blue", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30),
     xlab=" Number of Units Surveyed",
     ylab="Species Richness",
     main="Wetland Breeding Bird Survey 2020")
...
The trend in detection rate of species during the first and second survey periods was consistent across years for surveys completed on the 27 nonrandom units (Figure 14). Species richness increased quickly with survey effort during the first period and more slowly during the second period, based on the slope of each curve. Although the overall trend was consistent among years, 2017 experienced the lowest species richness during the first survey period, while 2020 experienced the lowest species richness during the second survey period. These curves do not take into account any differences in observer effort or bird identification skill. Each year of surveys, at least one observer was unique, meaning that they did not survey in any other years. \par 

\begin{figure}[]

\caption[Species accumulation curves representing avian detections during the early period (left) and late period (right) of wetland breeding bird surveys in 2016, 2017, 2019, and 2020.]{Species accumulation curves representing avian detections during the early period (left) and late period (right) of wetland breeding bird surveys in 2016, 2017, 2019, and 2020.}

```{r plot2, out.width="50%", eval=TRUE, echo=FALSE, message=FALSE}
...
accurve1a<-specaccum(spacc1.2016, method="collector", permutations=100)
accurve2a<-specaccum(spacc1.2017, method="collector", permutations=100)
accurve3a<-specaccum(spacc1.2019, method="collector", permutations=100)
accurve4a<-specaccum(spacc1.2020, method="collector", permutations=100)
...
accurve1b<-specaccum(spacc2.2016, method="collector", permutations=100)
accurve2b<-specaccum(spacc2.2017, method="collector", permutations=100)
accurve3b<-specaccum(spacc2.2019, method="collector", permutations=100)
accurve4b<-specaccum(spacc2.2020, method="collector", permutations=100)

################ Plots ##############################

plot(accurve1a, lty= 1, ci.type="line", col="black", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),
     xlab=" Number of Units Surveyed",
     ylab="Species Richness",
     main="Survey Period 1")
plot(accurve2a, lty= 1, ci.type="line", col="green", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),add = TRUE)
plot(accurve3a, lty= 1, ci.type="line", col="blue", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),add = TRUE)
plot(accurve4a, lty= 1, ci.type="line", col="red", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),add = TRUE)
legend(x=20,y=20,c("2016","2017","2019","2020"),cex=.8,col=c("black","green", "blue", "red"), lty = 1:1, box.lty=0)

plot(accurve1b, lty= 1, ci.type="line", col="black", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),
     xlab=" Number of Units Surveyed",
     ylab="Species Richness",
     main="Survey Period 2")
plot(accurve2b, lty= 1, ci.type="line", col="green", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),add = TRUE)
plot(accurve3b, lty= 1, ci.type="line", col="blue", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),add = TRUE)
plot(accurve4b, lty= 1, ci.type="line", col="red", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),add = TRUE)
legend(x=20,y=20,c("2016", "2017", "2019", "2020"),cex=.8,col=c("black","green", "blue", "red"), lty = 1:1, box.lty=0)
...
\caption[Species rarefaction curves representing two survey periods during wetland bird surveys in 2020.]{Species rarefaction curves representing two survey periods of wetland bird surveys in 2020.}

```{r plot3, out.width="90%", eval=TRUE, echo=FALSE, message=FALSE}
...
plot(specaccum(rare1, method = "rarefaction"), 
     xvar = "individuals", lty= 1, ci.type="line", col="blue", lwd=2, ci.lty=0, ci.col="lightblue",  xlab = "Number of Individuals", ylab="Species Richness", main= "Rarefaction Curves 2020")
...
\caption[Species diversity (Shannon Diversity Index) at 10 point count locations in the proposed Eagle River Flats expansion area.]{Species diversity (Shannon Diversity Index) at 10 point count locations in the proposed Eagle River Flats expansion area.}

```{r table3, out.width="60%", eval=TRUE, echo=FALSE, message=FALSE}
...
```{r calc2, eval=TRUE, echo=FALSE, message=FALSE, include= FALSE}
...
\caption[Average species richness detected at point count locations as a function of distance to Eagle River Flats and habitat classification.]{Average species richness detected at point count locations as a function of distance to Eagle River Flats and habitat classification.}

```{r Figure4, out.width="50%", eval=TRUE, echo=FALSE, message=FALSE}
...
ggplot(data = erf.rich, 
  ...
  scale_shape_manual(values=c(25, 21, 23))+
        ...
        ggtitle("Distance to Eagle River Flats")+
        theme_minimal()
...
ggplot(data = hab.data, 
        ...
        ylab("Ave. Species Richness")+
        ggtitle("Habitat classification (100m radius")+
        theme_minimal()
...
\caption[Common name, scientific name, and 4-letter species code for all birds present on Joint Base Elmendorf-Richardson.]{Common name, scientific name, and 4-letter species code for all birds present on Joint Base Elmendorf-Richardson.}
```{r appendixa, out.width="90%", eval=TRUE, echo=FALSE, message=FALSE}
...
![](appendixa.png){width=80%}

## Appendix B

```{r appendb, eval=TRUE, echo=FALSE, message=FALSE, results='hide'}
...
\caption[Status, number of individuals, and relative occurrence of bird species detected during two breeding season survey periods on lowland wetlands on Joint Base Elmendorf-Richardson, 2020.]{Status, number of individuals, and relative occurrence of bird species detected during two breeding season survey periods on lowland wetlands on Joint Base Elmendorf-Richardson, 2020.}

```{r appendixb, out.width="90%", eval=TRUE, echo=FALSE, message=FALSE}
...
kable(format="html",
 ...
 as_image(results.table,file="appendixb.png", width = 18)
```
...
\caption[Summary of all Lesser Yellowlegs adults banded and resighted on Joint Base Elmendorf-Richardson, 2016-2020]{Summary of all Lesser Yellowlegs adults banded and resighted on Joint Base Elmendorf-Richardson, 2016-2020}

```{r append1, out.width="60%", eval=TRUE, echo=FALSE, message=FALSE}
...
as_image(band,file="appendixC.png", width = 4)
