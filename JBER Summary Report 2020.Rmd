...
ARU &nbsp; &nbsp; &nbsp;&nbsp;&nbsp Automated Recording Unit  
BBA &nbsp; &nbsp; &nbsp;&nbsp;&nbsp; Breeding Bird Atlas  
BBS &nbsp; &nbsp; &nbsp;&nbsp;&nbsp; Breeding Bird Survey  
BCR &nbsp; &nbsp; &nbsp;&nbsp;&nbsp; Bird Conservation Region  
BPIF &nbsp; &nbsp; &nbsp;&nbsp;&nbsp; Boreal Partners in Flight  
CEIEC &nbsp;&nbsp;&nbsp;Natural Resources Element, Conservation and Compliance  
CES &nbsp; &nbsp; &nbsp;&nbsp;&nbsp;Civil Engineer Squadron  
DoD &nbsp; &nbsp; &nbsp;&nbsp;&nbsp;Department of Defense  
DOI &nbsp; &nbsp; &nbsp;&nbsp;&nbsp;Department of Interior  
ERF &nbsp; &nbsp; &nbsp; &nbsp; Eagle River Flats (Joint Base Elmendorf-Richardson)  
GPS &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Global Positioning System  
JBER &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  Joint Base Elmendorf-Richardson  
MBM &nbsp; &nbsp; &nbsp; &nbsp;  Migratory Bird Management (U.S. Fish and Wildlife Service)  
...
![Point count locations within the Eagle River Flats proposed expansion area](./ERFExpansion.jpg){width=80%}
...
![ARU locations on Eagle River Flats in 2019 and 2020. Two units were placed in the same location on the eastern edge of Eagle Rier Flats in both years](./ARU_locations.jpg){width=80%}
...
![Recently hatched Lesser Yellowlegs chicks in nest bowl at Ammo Country wetlands](./LEYEnest.jpg){width=80%}

\par

![Placemnet of TinyTag data logger probe in the center of a Lesser Yellowlegs nest ](./nest_probe.jpg){width=80%}
...
![Leg bands: Metal, plastic and alphanumeric flags](./bands.jpg){width=80%}

![Measuring the diagonal tarsus of a Lesser Yellowlegs](./LEYEmorph.jpg){width=80%}
...
![Blackpoll Warbler are a Neotropical migrant that breeding in the boreal forests of Alaska and Canada. This is a late arrive species that was only detected during the second survey period. The male's striking black and white plumage is contrary to the females' monocramatic olive-green plumage](./blpw.jpg){width=80%}

```{r eval=TRUE}
spacc1.2020<- read.csv("specaccum1_2020.csv", header = TRUE)
spacc2.2020<- read.csv("specaccum2_2020.csv", header = TRUE)

s1 <-colSums(spacc1.2020, na.rm = FALSE, dims = 1)
s1[order(-s1)]

s2 <-colSums(spacc2.2020, na.rm = FALSE, dims = 1)
s2[order(-s2)]
...
sosc<- read.csv("SOSC_table.csv", header = TRUE)
...
![Blackpoll Warbler and Rusty Blackbird nest within mixed forests along the edges of wetlands](./blpw_habitat.jpg){width=80%}

![Sandpiper species forage in the emergent vegetation along the edges of lakes and ponds.](./leye_habitat.jpg){width=80%}
...
erf.sp<- read.csv("erfSpecies_table.csv", header = TRUE)
...
![A failed Lesser Yellowlegs nest found on the Ammo Country wetlands at Joint Base Elmendorf-Richardson, 2020. The 4mm diameter fissures and egg frangments suggest the nest was depredated by an avian predator.](./amco_egg.jpg){width=80%}
...
spacc1.2016<- read.csv("specaccum1_2016.csv", header = TRUE)
spacc1.2017<- read.csv("specaccum1_2017.csv", header = TRUE)
spacc1.2019<- read.csv("specaccum1_2019.csv", header = TRUE)
spacc1.2020<- read.csv("specaccum1_2020.csv", header = TRUE)
...
plot(accurve1, lty= 1, ci.type="line", col="black", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30),
     xlab=" Number of Units Surveyed",
     ylab="Species Richness",
     main="Figure XX: Survey Period 1")
...
spacc2.2016<- read.csv("specaccum2_2016.csv", header = TRUE)
spacc2.2017<- read.csv("specaccum2_2017.csv", header = TRUE)
spacc2.2019<- read.csv("specaccum2_2019.csv", header = TRUE)
spacc2.2020<- read.csv("specaccum2_2020.csv", header = TRUE)
...
plot(accurve1, lty= 1, ci.type="line", col="black", lwd=2, ci.lty=0, ci.col="lightblue", xlim=c(0,30), ylim=c(0,60),
     xlab=" Number of Units Surveyed",
     ylab="Species Richness",
     main="Figure XX: Survey Period 2")
...
##### Rarefaction curve ###########################################################
rare1<- read.csv("rarefaction1_2020.csv", header = TRUE)
rare2<- read.csv("rarefaction2_2020.csv", header = TRUE)

plot(specaccum(rare1, method = "rarefaction"), 
     xvar = "individuals", lty= 1, ci.type="line", col="blue", lwd=2, ci.lty=0, ci.col="lightblue",  xlab = "Number of Individuals", ylab="Species Richness", main= "Figure XX: Rerfaction Curves 2020")
...
```{r eval=TRUE, echo=FALSE, message=FALSE}
taxon<- read.csv("yeartrend_taxon.csv", header = TRUE)
...
ggplot(taxon, aes(x=year, y=n, group=taxon, colour=taxon)) +
    ...
    xlab("year") +
    ggtitle("Figure XX: Detection Trends")+
...
erf.rich<- read.csv("spec.rich.csv", header = TRUE)

diverse1<- read.csv("erfdiv_2020.csv", header = TRUE)
...
ggplot(data = erf.rich, 
  ...
  scale_shape_manual(values=c(25, 21, 23))+
        ...
        ggtitle("Figure XX: Point Count Species Richness as a funtion of distance to Eagle River Flats")+
        theme_minimal()
...
ggplot(data = hab.data, 
        ...
        ylab("Ave. Species Richness")+
        ggtitle("Figure XX: Point Count Species Richness as a function of habitat classification (100m radius")+
        theme_minimal()
...
## References
Andres, B. A. 2005. Breeding bird atlas of Fort Richardson, Alaska. Unpublished report, U. S. Fish and Wildlife Service. Migratory Bird Management, Anchorage, Alaska. 85 pp.
Boreal Partners in Flight. 2017. In prep. Alaska Landbird Conservation Plan. Anchorage, AK. Browne, B. T. and B. A. Andres. 2002. LCTA breeding bird survey Fort Richardson, Alaska, 1995-1997. Unpublished report, U.S. Fish and Wildlife Service, Anchorage, AK. 12 pp.
Cowardin, L. M., V. Carter, F. C. Golet, and E. T. LaRoe. 1979. Classification of wetlands and deepwater habitats of the United States. U.S. Fish and Wildlife Service, Biological Services Program (FWS/OBS-79/31), Washington, DC.
Department of Defense Partners in Flight Program. 2015. Strategic Plan for Bird Conservation and Management on Department of Defense Lands. https://www.denix.osd.mil/nr/priorities/birds/factsheets/dod-pif-mission-sensitive- priority-bird-species-october-2015-legacy-14-1717/
Handel, C.M., J.R. Sauer. 2017. Combined analysis of roadside and off-road breeding bird survey data to assess population change in Alaska. Condor 119:557-575.
Hutt, M. B. 1991. Shooting of migrating shorebirds in Barbados. ICBP Technical Publication 12:77-91.
Jorgenson, M. T., J. E. Roth, M. K. Raynolds, M. D. Smith, W. Lentz, and A. L. Zusi-Cobb.
1998. An ecological land classification for Fort Wainwright, Alaska. Final report for U.S. Army CRELL, Hanover, NH. Alaska Biological Research, Inc., Fairbanks, AK.
Matsuoka, S. M., Shaw, D., and Johnson, J. A. 2010a. Estimating the abundance of nesting Rusty Blackbirds in relation to wetland habitats in Alaska. Condor 112:825-833.
Matsuoka, S. M., D. Shaw, P. H. Sinclair, J. A. Johnson, R. M. Corcoran, N. C. Dau, P. M. Myers, and N. A. Rojek. 2010b. Nesting ecology of the Rusty Blackbird in Alaska and Canada. Condor 112:810-824.
MWH Americas, Inc. 2015. Species of special concern 2014 survey report. Prepared for Joint
Base Elmendorf-Richardson, 673d CES/CEIEC. Unpublished report. 12 pp.
Rappole, J. H. and Tipton, A. R. 1991. New harness design for attachment of radio transmitters to small passerines. J. Field Orn.62:335-337.
Rosenberg, K. V., Dokter, A. M., Blancher, P. J., Sauer, J. R., Smith, A. C., Smith, P. A., Stanton, J. C., Panjabi, A., Helft, L., Parr, M., and Marra, P. P. 2019. Decline of North American avifauna. Science 366:120-124.
Rosenberg, K. V., Kennedy, J. A., Dettmers, R., Ford, R. P., Reynolds, D., Alexander, J. D., Beardmore, C. J., Blancher, P. J., Bogart, R. E., Butcher, G. S., Camfield, A. F., Couturier, A., Demarest, D. W., Easton, W. E., Giocomo, J. J., Keller, R. H., Mini, A. E.,
Panjabi, A. O., Pashley, D. N., Rich, T. D., Ruth, J. M., Stabins, H., Stanton, J., and Will,
T. 2016. Partners in Flight Landbird Conservation Plan: 2016 Revision for Canada and Continental United States. Partners in Flight Science Committee. 119 pp.
Sauer, J. R., D. K. Niven, J. E. Hines, D. J. Ziolkowski, Jr, K. L. Pardieck, J. E. Fallon, and W.
A. Link. 2017. The North American Breeding Bird Survey, Results and Analysis 1966 - 2015. Version 2.07.2017 USGS Patuxent Wildlife Research Center, Laurel, MD.
...
U.S. Shorebird Conservation Plan Partnership. 2016. U.S. shorebirds of conservation concern 2016. http://www.shorebirdplan.org/science/assessment-conservation-status-shorebirds/. Assessed September 2017.
Watts, B. D., E. T. Reed, C. Turin. 2015. Estimating sustainable mortality limits for shorebirds using the Western Atlantic Flyway. Wader Study 122:37-53.

## Appendix A: Species List

```{r eval=TRUE, echo=FALSE, message=FALSE}

spec<- read.csv("species_table.csv", header = TRUE)
...
```{r eval=TRUE, echo=FALSE, message=FALSE}
results<- read.csv("jber_lowland_survey_2020.csv", header = TRUE)
...
results.table<- read.csv("results_table.csv", header = TRUE)
...
![GPS locations of Lesser Yellowlegs on Joint Base Elmendorf-Richardson by year (top) and individual (bottom).](./leye_erflocs.jpg){width=80%}
...
![Lesser Yellowlegs banded with alphanumeric flag "TY" on Joint Base Elmendorf-Richardson, 2020.](./leye_flag.jpg){width=80%}
...
band<- read.csv("band_table.csv", header = TRUE)
...
![A Lesser Yellowlegs carrying a PinPoint Argos GPS tag on lle Niapiskau (Mingan Archipeligo), Quebec, 2020. Photo: Y. Rochepault.](./LEYE_Mingan.jpg){width=80%}
...
![Migratory track lines for Lesser Yellowlegs tagged with Lotek PinPoint GPS Argos Satellite tags, 2018-2020.](./leye_map.jpg){width=80%}
...
![Fall migratory stopover locations for Lesser Yellowlegs fitted with PinPoint Argos GPS tags. Blue polygons represent U.S. military installations. These were the individuals closest to military installations in 2018-2020.](./mil_map.jpg){width=80%}
